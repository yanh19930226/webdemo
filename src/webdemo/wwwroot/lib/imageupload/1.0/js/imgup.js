...
(function ($) {
    ...
    $.fn.imageUpload = function (option, param) {
        ...
        if (_option.canAdd == 1) {
            html += '           <img src="/lib/imageupload/1.0/img/add.png" class="add-img">';
            deleteDisplay = 'block';
        }
        ...
        setImageUrl: function (target, imageUrl) {
            if (imageUrl) {
                ...
                for (var i = 0; i < urlArr.length; i++) {
                    if (urlArr[i] != "") {
                        var deleteId = Math.floor(Math.random() * 16.0).toString(16);
                        var imageName = urlArr[i].substring(urlArr[i].lastIndexOf('/') + 1);
                        var html = '';
                        html += '<section class="up-section fl">';
                        html += '   <span class="up-span"></span>';

                        html += '   <img id="' + deleteId + '" class="close-upimg" style="display:' + deleteDisplay + '" src="/lib/imageupload/1.0/img/delete.png" />';
                        if (urlArr[i].indexOf('http') > -1 || urlArr[i].indexOf('https') > -1) {
                            html += '   <img class="up-img" src="' + urlArr[i] + '" />';
                        }
                        else {
                            html += '   <img class="up-img" src="' + context + urlArr[i] + '" />';
                        }
    ...
    function prepareUploadImage(inputFileId) {
        ...
        if (fileList.length > limit || totalNum > limit) {
            ...
            var uploadSuccess = function (imgPath) {
                ...
                var re = imgPath.substring(imgPath.lastIndexOf('/') + 1);
